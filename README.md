...
- **Storage:** Templates and outputs are stored in **S3-compatible object storage** under `aiTaras/…` and served by backend API routes.
- **Server env:** set `ANTHROPIC_API_KEY`, S3 variables (`S3_BUCKET`, `S3_REGION`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_PUBLIC_BASE_URL`, optional `S3_ENDPOINT`) and optional `TARAS_ANTHROPIC_MODEL` in `server/.env`. Deploy Firestore indexes from `firestore.indexes.json` after the first `collectionGroup` query error link, if needed.
